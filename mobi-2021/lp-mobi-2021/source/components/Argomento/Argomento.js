...
export function Argomento({ className, classes, ...props }) {
	...
	const renderPicture = (sources, index = 0) => {
		return (
			<Picture className={cls(classes.framePicture)} classNameImg={cls(classes.framePictureImg)} sources={sources} />
		);
	};
	...
	const { mini, num, useNum, usePlural, sources, title, subtitle, children, ...rootProps } = props;
	return (
		<div
			...
			<div className={cls(classes.frame)}>
				<div className={cls(classes.frameOuter)}>
					<div className={cls(classes.frameInner)}>{renderPicture(sources)}</div>
					<Fade bottom>
