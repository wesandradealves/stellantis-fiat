...
const cssRegex = /\.global\.css(\?.*)?$/i;
const cssModuleRegex = /.*(?<!\.global)\.css(\?.*)?$/i;
const sassRegex = /\.global\.(?:scss|sass)(\?.*)?$/i;
const sassModuleRegex = /.*(?<!\.global)\.s[ac]ss(\?.*)?$/i;
...
const getCSSModuleLocalIdent = (ctx, localIdentName, localName) => localName.replace(/_-/g, '');
module.exports = function(webpackEnv) {
  ...
  const sassOptions = {
    outputStyle: isEnvProduction ? 'compressed' : 'expanded',
    includePaths: [
      paths.appNodeModules,
      paths.appSrc,
    ],
    functions: {
      'js($keys)': sassAidFunctionJS(process.env),
    },
  };
    ...
    module: {
      ...
      rules: [
        ...
        },
        {
          oneOf: [
						{
							test: /\.(?:glsl|frag(?:ment)?|fs|vert(?:ex)?|vs)(?:(?:\?|#).*)?$/i,
							exclude: /^(?:node_modules|bower_components)$/,
							use: [
								{
									loader: require.resolve('glslify-import-loader'),
								},
								{
            ...
            },
            {
              ...
              use: getStyleLoaders({
                ...
                sourceMap: isEnvProduction && shouldUseSourceMap,
								...
								import: true,
                modules: {
									localIdentName: isEnvProduction ? '[name][local]--[hash:base64:5]' : '[name][local]',
