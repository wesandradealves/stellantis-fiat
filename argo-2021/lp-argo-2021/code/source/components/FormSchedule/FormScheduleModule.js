...
import { findCityByName, getCookie, getCookieDirect } from './FormScheduleUtils';
...
const setCity = [
	...
	function({ props }) {
		...
		let influenceAreas;
		if (props.cityData) {
			influenceAreas = props.cityData.influenceAreas.filter((x) => x.brand.id === idBrand);
		} else {
			influenceAreas = props.influenceAreas.filter((x) => x.brand.id === idBrand);
		}
...
function formatPhone(ddd, phone) {
	return `(${ddd}) ${phone.substring(0, 4)}-${phone.substring(4, phone.length)}`.substring(0, 40);
}
...
export default Module({
	...
	signals: {
		submitFormToBot: [
			({ module }) => {
				const { model, version, serie } = formatMVS(module.get('form.typeVersion.value'));
				...
				if (module.get('form.ddd1.value') !== '') {
					data[getPhoneFieldToBot(module.get('form.phoneType1.value'))] = formatPhone(
		...
		submitForm: [
			({ module }) => {
				...
				let data = {
					...
					ga_transaction_id: new Date().getTime() + module.get('form.cpfCnpj.value'),
					...
					term: module.get('form.term.value'),
					...
					clientId: (document.cookie.match(/\bIsobar_ClientID=([^;]*)/) || [])[1] || null,
					description: module.get('form.message.value'),
				};

				if (module.get('form.ddd1.value') != '') {
					data[getPhoneField(module.get('form.phoneType1.value'))] = formatPhone(
						module.get('form.ddd1.value'),
						module.get('form.phone1.value'),
					);
				}
				if (module.get('form.ddd2.value') != '') {
					data[getPhoneField(module.get('form.phoneType2.value'))] = formatPhone(
						module.get('form.ddd2.value'),
						module.get('form.phone2.value'),
					);
				}
				if (module.get('form.ddd3.value') != '') {
					data[getPhoneField(module.get('form.phoneType3.value'))] = formatPhone(
						module.get('form.ddd3.value'),
						module.get('form.phone3.value'),
					);
				}

				const url = 'https://lead.fcalatam.com.br/HubMicroservices';
		...
		addPhoneValidation: [
			({ props, state }) => {
				...
				state.set('formSchedule.form.ddd' + index + '.validationRules', ['isRequired']);
				state.set('formSchedule.form.phone' + index + '.validationRules', ['isRequired']);
			},
		],
