...
export const dataTypeVersion = [
	{ value: 'ACN', name: 'ARGO 1.0 FLEX 4P 2022' },
    { value: 'AFN', name: 'ARGO DRIVE 1.0 FLEX 4P 2022' },
    { value: 'AFH', name: 'ARGO DRIVE 1.3 FLEX 4P 2022' },
    { value: 'AGH', name: 'ARGO TREKKING 1.3 FLEX 4P 2022' },
];
...
export class FormSchedule extends React.Component {
	...
	componentDidMount() {
		...
		const locationSearch = window.location.hash ? window.location.hash.split('?')[1] : '';
    ...
    const utmValues = {
      ...
      source:
        ...
        '',
      term:
        queryString.get('utmTerm') ||
        queryString.get('utm_term') ||
        '',
    };
	...
	renderPhones = () => {
		...
		const showRemove = (phone1 ? 1 : 0) + (phone2 ? 1 : 0) + (phone3 ? 1 : 0) > 1;
		const list = [1, 2, 3];
		return list.map((v, k) => {
			...
			return (
				<div className={cls(classes.row, classes.contentPhone)} key={k}>
					<div className={cls(classes.gr6, classes.gr4Mobile, classes.inputContainer)}>
						<InputConnect filter={(v) => v.substr(0, 2)} path={`formSchedule.form.ddd${v}`}>
							<input
								className={cls(classes.size24, classes.size26Mobile, classes.ddd)}
								placeholder='DDD'
								maxLength={2}
								type='number'
							/>
						...
						<InputConnect tracking='telefone' filter={(v) => v.substr(0, 9)} path={`formSchedule.form.phone${v}`}>
							<input
								className={cls(classes.size24, classes.size26Mobile, classes.phone)}
								placeholder='Número'
								maxLength={9}
								type='number'
							/>
					...
					<div
						...
						{showRemove && <div className={cls(classes.btnRemove)} onClick={this.onRemovePhoneClick.bind(this, v)} />}
					</div>
				</div>
			);
		});
	...
	renderConcessionarias = () => {
		if (this.props.isLoadingDealers) {
			return (
				<div className={cls(classes.gr12, classes.gr4Mobile)}>
					<div className={cls(classes.txtLoading, classes.size24, classes.uBold, classes.size22Mobile)}>Carregando</div>
				</div>
			);
		}
		if (this.props.dealers.length == 0) {
			return (
				<div className={cls(classes.gr12, classes.gr4Mobile)}>
					<div className={cls(classes.txtLoading, classes.size24, classes.uBold, classes.size22Mobile)}>
						Não foram encontradas concessionárias em {this.props.cityData.cityName}
					</div>
				</div>
			);
		}
		return this.props.dealers.map((v, k) => {
			return (
				<InputConnect key={k} path='formSchedule.form.concessionaria' onChange={this.onChangeConcessionaria}>
	...
	renderSuggestions = () => {
		...
		return (
			<ul
				...
				{suggestions.map((x, i) => (
					<li className='autocomplete-item' key={i} onClick={() => this.suggestionsSelected(x)}>
	...
	suggestionsSelected = (params) => {
		...
		this.setState({ suggestions: [], value: params.cityName + ' - ' + params.stateCode });
	};
					...
					<div className='alert'>
						<div className='textAlert'>
							<p className='size-14 size-26-mobile u-light text alert-text'>
								Visando a comodidade de todos, informamos que nossas concessionárias estão com horários reduzidos ou
								inoperantes. Caso necessite de algum serviço ou informação, pedimos que entre em contato com a
								concessionária de seu interesse ou envie seus dados que retornaremos assim que possível.
							</p>
						</div>

						<div className='button'>
							{this.state.isActive ? (
								<a className='btn-send btn-upper u-bold size-24 size-26-mobile' onClick={this.handleHide}>
									Entendi
								</a>
							) : (
								<a className='btn-send btn-upper u-bold size-24 size-26-mobile' onClick={this.handleShow}>
									Entendi
								</a>
							)}
						</div>
	...
	renderAlert() {
		return (
			<div className='alert'>
				<div className='textAlert'>
					<p className='size-14 size-26-mobile u-light text alert-text'>
						Visando a segurança de todos, informamos que nossas concessionárias estão com horários reduzidos ou
						inoperantes. Caso necessite de algum serviço ou informação, pedimos que entre em contato com a
						concessionária de seu interesse ou envie seus dados que retornaremos assim que possível.
					</p>
				</div>
				<div className='button'>
					<a className='btn-send btn-upper u-bold size-24 size-26-mobile' onClick={this.hide}>
						Entendi
					</a>
				</div>
			</div>
			...
			<div className={cls(classes.root, className)}>
				{this.props.responseData && (
					<div className={cls(classes.feedbackModal)}>
						<div className={cls(classes.block, classes.size24, classes.size24Mobile)}>
							<div className={cls(classes.txtStatus)} tracking='lead-gerado'>
							{this.props.responseData
                  ? 'Formulário enviado. Aguarde que em breve entraremos em contato.'
				...
				<DataLayer onEvent={'FormSchedule_ClicouFechar'}>
					<Link to={`/${window.location.search || ''}`} onClick={this.props.toggleForm} className={cls(classes.btnClose)} tracking='fechar'>
						<img alt='Fechar' src={require('./assets/btn-close.png')} />
					</Link>
				</DataLayer>
				<div className={cls(classes.container, classes.size14, classes.size20Mobile)}>
					<div className={cls(classes.row)}>
						<div
							...
							<div className={cls(classes.size36, classes.size36Mobile, classes.uBold, classes.title)}>
								Preencha o formulário abaixo e prepare-se para se surpreender com todos os detalhes do seu próximo Fiat
								Argo.
							</div>
							<div className={cls(classes.size24, classes.size26Mobile, classes.uLight, classes.text)}>
								Fique tranquilo, um de nossos vendedores entrará em contato com você.
							</div>
							<div className={cls(classes.row)}>
								<div className={cls(classes.gr5, classes.suffix1, classes.gr4Mobile, classes.suffix0Mobile)}>
									<InputContainer label='NOME COMPLETO'>
										<InputConnect tracking='preencheu-nome' path='formSchedule.form.name'>
											<input maxLength={60} className={cls(classes.size24, classes.size26Mobile)} />
										</InputConnect>
										<ErrorMessage path='formSchedule.form.name' />
									</InputContainer>
									<div className={cls(classes.separator)} />
									<InputContainer label='E-MAIL'>
										<InputConnect tracking='preencheu-email' path='formSchedule.form.email'>
											<input maxLength={60} className={cls(classes.size24, classes.size26Mobile)} />
										</InputConnect>
									...
									<div className={cls(classes.phones)}>
										<div className={cls(classes.row)}>
											<div className={cls(classes.gr6, classes.gr4Mobile)}>
												<InputContainer label='TELEFONE' tracking='preencheu-telefone' />
											</div>
										</div>
										{this.renderPhones()}
										{(!phone1 || !phone2 || !phone3) && (
											<div
												className={cls(classes.size22, classes.size26Mobile, classes.uLight, classes.btnAddPhone)}
												onClick={this.onAddPhoneClick}
												tracking='add-telefone'>
												<img alt='Adicionar telefone' src={require('./assets/add.png')} /> Adicionar telefone
											</div>
										)}
									</div>
								</div>
								...
								<div className={cls(classes.gr6, classes.gr4Mobile)}>
									<div className={cls(classes.row)}>
										<div className={cls(classes.gr5, classes.gr4Mobile)}>
											<InputContainer label='SELECIONE SEU PERFIL'>
												<InputConnect
													onChange={(evt) => {
														DataLayer.push('FormSchedule_PreencheuPerfil', evt.target.value);
													}}
													path={'formSchedule.form.typePerson'}
													tracking='preencheu-perfil'>
													<Select data={dataTypePerson} placeholder='Selecione seu perfil' />
												</InputConnect>
												<ErrorMessage path='formSchedule.form.typePerson' />
											</InputContainer>
										</div>
										{media.mobile && <div className={cls(classes.separator)} />}
										<div className={cls(classes.gr5, classes.prefix1, classes.gr4Mobile, classes.prefix0Mobile)}>
											<InputContainer label='CPF/CNPJ'>
												<InputConnect
													...
													<input
														className={cls(classes.size24, classes.size26Mobile, classes.hasMask)}
														maxLength={18}
														type='text'
													/>
												</InputConnect>
									...
									<div className={cls(classes.___)}>
										<InputContainer label='SELECIONE UMA VERSÃO'>
											<InputConnect
												onChange={(evt) => {
													DataLayer.push('FormSchedule_PreencheuVersao', evt.target.value);
												}}
												tracking='preencheu-versao'
												path={'formSchedule.form.typeVersion'}>
												<Select data={dataTypeVersion} placeholder='Selecione uma versão' />
											</InputConnect>
									...
									<InputContainer label='MENSAGEM'>
										<InputConnect path={'formSchedule.form.message'} tracking='preencheu-mensagem'>
											<textarea
												maxLength={300}
												className={cls(classes.textArea, classes.size24, classes.size26Mobile)}
				...
				{this.props.cityData && (
					<div className='form-two'>
						<div className='container'>
							<div className='size-36 size-36-mobile u-bold'>Escolha uma das concessionárias abaixo</div>
							...
							<div className='row concessionarias'>{this.renderConcessionarias()}</div>
							...
							{!this.props.isLoading && (
								<div className={'btn-wrapper'}>
									...
									<div className='btn-container'>
										<div
											className={cls('btn-send', 'u-bold', 'size-24', 'size-26-mobile', {
										...
										<DataLayer onEvent={'FormSchedule_ClicouWhatsApp'}>
											<a
												className={cls('btn-whatsapp', 'u-bold', 'size-24', 'size-26-mobile', {
													['is-disabled']: !formSchedule.isValid,
												})}
												href={this.getLinkWhatsApp(this.state.dealerPhone, 'Quero meu FIAT 0km')}
												target='_blank'
												onClick={() => this.onClickSend(true)}>
												<i className={cls('fa', 'fa-whatsapp')} /> FALE COM UM ASSISTENTE
											</a>
										</DataLayer>
									</div>
								</div>
							)}
							{this.props.isLoading && <div className='btn-send u-bold size-24 size-26-mobile'>ENVIANDO...</div>}
						</div>
					</div>
				)}
			</div>
