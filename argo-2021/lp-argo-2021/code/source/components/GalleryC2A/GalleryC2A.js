...
export class GalleryC2A extends React.Component {
	...
	componentDidMount() {
		...
		this.timeline.add([
			TweenMax.to(this.refs.root, 1, { autoAlpha: 1, /*y: 0,*/ ease: Quart.easeInOut }),
			TweenMax.from(this.refs.text, 1, { autoAlpha: 0, y: 20, ease: Back.easeOut, delay: 0.5 }),
			TweenMax.from(this.refs.callToAction, 1, { autoAlpha: 0, y: 20, ease: Back.easeInOut, delay: 0.55 }),
			TweenMax.from(this.refs.closeButton, 1, { autoAlpha: 0, ease: Back.easeInOut, delay: 0.6 }),
		]);
	}
	...
	hide = () => {
		TweenMax.to(this.refs.root, 0.5, { autoAlpha: 0, /*y: 0,*/ ease: Quart.easeInOut });
	};
			...
			<div className={cls(classes.root, className)} ref='root'>
				<div className={cls(classes.container)}>
					...
					<DataLayer onEvent={'Galeria_C2A_Fechou'}>
						<button className={classes.closeButton} onClick={this.hide} ref='closeButton'>
							x
						</button>
					</DataLayer>
					<DataLayer onEvent={'Galeria_C2A_LinkAgende'}>
						<Link className={classes.callToAction} to={`/agende${window.location.search || ''}`} ref='callToAction'>
							COMPRE O SEU
						</Link>
					</DataLayer>
				</div>
