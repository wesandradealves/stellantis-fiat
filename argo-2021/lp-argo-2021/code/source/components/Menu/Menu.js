...
import { ArgoBrand } from '~/components/ArgoBrand';
...
export class Menu extends React.Component {
		...
		if ($target) {
			...
			this.scrollTo(scrollTo + 60, 0.5, {
				onComplete: () => {
					...
					const vpv = /design|performance|tecnologia|seguranca/gi.test(uiAnchor) ? 'sobre' : uiAnchor;
	...
	componentDidMount() {
		...
		Array.prototype.forEach.call(anchors, ($el, index) => {
			...
			this.sections[anchorName] = { index, anchor: anchorName, y: $el.offsetTop };
		});
	...
	renderNavButton = (item, index) => {
		return (
			<DataLayer key={index} onEvent={item.dataLayerId || 'Menu_ClicouItem'} args={[item.label]}>
				<button
					...
					<span
						...
						dangerouslySetInnerHTML={{ __html: this.props.mobile ? item.labelMobile || item.label : item.label }}
					/>
				</button>
			</DataLayer>
		);
	...
	renderNavAnchor = (item, index) => {
		return (
			<DataLayer key={index} onEvent={item.dataLayerId || 'Menu_ClicouItem'} args={[item.label]}>
				<a className={cls(classes.asideAnchorLink)} href={item.href} target={item.target}>
					{item.icon ? <img alt={`menu-${item.href}`} src={item.icon} /> : ''}
					<span
	...
	renderNavLink = (item, index) => {
		return (
			<DataLayer key={index} onEvent={item.dataLayerId || 'Menu_ClicouItem'} args={[item.label]}>
				<Link className={cls(classes.asideAnchorLink)} to={`${item.href}${window.location.search || ''}`}>
					<span
						className={cls(classes.asideAnchorLinkText)}
						dangerouslySetInnerHTML={{ __html: this.props.mobile ? item.labelMobile || item.label : item.label }}
					/>
	...
	renderMenuItems = (renderType = '*') => {
		return this.props.data.list.map((item, index) => {
			...
			else if (item.href) return renderAnchors && this.renderNavAnchor(item, index);
		...
		return (
			<div className={cls(classes.navBar)}>
				...
				<DataLayer onEvent={'Menu_ClicouLogoFiat'} args={[]}>
					<button className={cls(classes.navBarLogo)} onClick={this.onClickLogo}>
						<FiatBrand className={cls(classes.navBarLogoIcon)} />
						{props.mobile && <span>FIAT ARGO</span>}
					</button>
				</DataLayer>
				...
				{props.data.pin.visible && (
					<DataLayer onEvent={'Menu_ClicouPin'} args={[]}>
						<a className={cls(classes.navBarPin)} href={props.data.pin.href} target={props.data.pin.target}>
	...
	renderAside = (isOpen, props) => {
		return (
			<div className={cls(classes.aside)}>
				{props.mobile && <CloseButton className={cls(classes.asideClose)} onClick={this.close} />}
				<div className={cls(classes.asideList)}>
					<div className={cls(classes.asideListAnchors)}>{this.renderMenuItems('button')}</div>
					<div className={cls(classes.asideListLinks)}>{this.renderMenuItems('link')}</div>
				</div>
			</div>
		);
	};
		...
		return (
			<div className={cls(classes.brand)}>
				...
				<DataLayer onEvent={'Menu_ClicouLogoCarro'} args={[]}>
					<span className={cls(classes.brandArgo)} onClick={this.onClickLogo}>
						FIAT ARGO
					</span>
				</DataLayer>
			</div>
	...
	render() {
		...
		this.anchorList = Array.prototype.slice.call(document.querySelectorAll('[data-ui-anchor]'));
		return (
			<div
				ref={'menu'}
				...
				{props.mobile ? this.renderMobile(isOpen, props) : this.renderDesktop(isOpen, props)}
			</div>
		);
	}
}
