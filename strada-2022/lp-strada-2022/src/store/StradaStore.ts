...
import { HeroProductTab, heroAllSlides } from '@/mocks/heroDetails';
...
export enum StorageNames {
  PRIVACY_POLICY = '@strada/privacy-policy',
}
...
class StradaStore {
  searchQuery: URLSearchParams | null = null;
  ...
  highlightedVersionsMobileSwiperController: SwiperCore | null = null;
  ...
  constructor() {
    makeObservable(this, {
      ...
      setSelectedHeroDetailTab: action,
  ...
  setFeaturesSwiperController = (s: SwiperCore | null): SwiperCore | null => this.featuresSwiperController = s;

  setFeaturesMobileSwiperController = (s: SwiperCore | null): SwiperCore | null => this.featuresMobileSwiperController = s;

  setNewProductsSwiperController = (s: SwiperCore | null): SwiperCore | null => this.newProductsSwiperController = s;

  setNewProductsMobileSwiperController = (s: SwiperCore | null): SwiperCore | null => this.newProductsMobileSwiperController = s;

  setExpandedGallerySwiperController = (s: SwiperCore | null): SwiperCore | null => this.expandedGallerySwiperController = s;

  setProductDetailsSwiperController = (s: SwiperCore | null): SwiperCore | null => this.productDetailsSwiperController = s;

  setProductDetailsMobileSwiperController = (s: SwiperCore | null, tabId: string): MobileSwiperController => {
  ...
  setHighlightedVersionsSwiperController = (s: SwiperCore): SwiperCore => this.highlightedVersionsSwiperController = s;

  setHighlightedVersionsMobileSwiperController = (s: SwiperCore): SwiperCore => this.highlightedVersionsMobileSwiperController = s;
  ...
  setVersionSlugSuffix = (s: string): string => this.versionSlugSuffix = s;

  setStopHistoryChange = (b: boolean): boolean => this.stopHistoryChange = b;

  setSlugSuffixes = (reference: MenuLabel, func: (slugSuffix: string) => void): void => {
    this.slugSuffixes = {
      ...this.slugSuffixes,
      [reference.slug]: func,
    }
  ...
  setFixModalHero = (b: boolean): boolean => this.fixModalHero = b;

  setSearchQuery = (s: URLSearchParams): URLSearchParams => this.searchQuery = s;

  setShowCookieInfo = (b: boolean): boolean => this.showCookieInfo = b;

  setShowDisclaimerInfo = (b: boolean): boolean => this.showDisclaimerInfo = b;
  ...
  swipeToTabHero = (tab: HeroProductTab): void => {
    if (this.featuresSwiperController) {
      const index = heroAllSlides.findIndex((s) => s.tabId === tab?.id);
      if (this.featuresSwiperController) {
        this.featuresSwiperController?.slideTo(index);
      }
    }
  }
  
  setSelectedHeroDetailTab = (t: HeroProductTab | null): string => {
    return this.selectedHeroDetailTab = t ? t.id : '';
  }

  setSelectedDetailTab = (t: ProductTab | null, setHistyState = true): string => {
    if (t && setHistyState) {
      const reference = getProductDetails(productDetails).find((p) => p.id === `data-${t?.id}`);
      const sufffix = allSlides.filter((tab) => tab.tabId === t.id)[0]?.slug;
      reference && t?.parentSlug && this.setCurrentlyVisibleNav(reference, sufffix);
    }
    return this.selectedDetailTab = t ? t.id : '';
  }

  setNavRef = (x: MenuLabel['slug'], ref: MutableRefObject<unknown>): NavRefs => {
    return this.navRefs = { ...this.navRefs, [x]: ref };
  }

  scrollToRef = (ref: MenuLabel): void => {
    const element = this.navRefs[ref.slug]?.current as HTMLDivElement;
    (element)?.scrollIntoView({ behavior: 'smooth' });
  }

  setCurrentlyVisibleNav = (v: MenuLabel, suffix = ''): MenuLabel['id'] => {
    const href = suffix ? `${v.slug}/${suffix}` : v.slug;
    if (window?.history && !this.stopHistoryChange) {
      try {
        const searchParams = this.searchQuery?.toString() ? `/?${this.searchQuery?.toString()}` : '';
        window.history.replaceState({ section: v }, '', `${process.env.BASE_PREFIX ?? '/'}${href}${searchParams}`.replace('//', '/'));
      } catch (_e) {
        console.error(_e);
      }
    }
  ...
  setModalHero = (b: BFunc): boolean => {
    const setProp: boolean = typeof b !== 'boolean' ? b(this.modalHero) : b;
    this.checkNoScroll(setProp);
  ...
  setFormSection = (f: HTMLElement | null): HTMLElement | null => this.formSection = f;
}


export default StradaStore;
