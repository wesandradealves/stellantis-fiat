...
const CustomerType: FC = observer(() => {
  ...
  const setReference = (index?: number) => {
    ...
    const suffix = cutomerType[activeIndex].slug;
    reference &&
  ...
  return (
    <SectionElement
      ...
      overrideReference
      onSlugSuffix={(slug) => {
        const referenceIndex = cutomerType.findIndex(
          (c) => c.slug === slug,
        );
    ...
    >
      ...
      >
        <h2>
          Está procurando o parceiro perfeito para o trabalho?
        </h2>
        <Chevron
